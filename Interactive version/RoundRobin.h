...
Gantt createRoundRobin(Process processes[],int numberOfProcesses,int timeQuantum){
    ...
    for(total = 0, i = 0; x != 0;){
            ...
            for(k=0;k<burst_times[i];k++){
                ...
                if(total !=0){
                    ...
                    for(theta =0;theta<numberOfProcesses;theta++){
                        strcpy(temp.queues[total].processes_waiting[theta].name,temp.queues[total - 1].processes_waiting[theta].name);
                    }
                    //Remove current process(used)
                    temp.queues[total].lastIndex = temp.queues[total-1].lastIndex;
                    if(strcmp(temp.queues[total].processes_waiting[0].name,"::") != 0 && change == 0){
                        int dex;
                        for(dex=0;dex<numberOfProcesses-1;dex++){
                            strcpy(temp.queues[total].processes_waiting[dex].name,temp.queues[total].processes_waiting[dex+1].name);
                        }
                        strcpy(temp.queues[total].processes_waiting[numberOfProcesses-1].name,"::");
                        //Current process removed from queue => change updated
                        change = 1;
                        //Decrement lastIndex(index of last process in queue)
                        temp.queues[total].lastIndex--;
                    }
                }
                //Update virtualCPU and States of Gantt chart to current process
                strcpy(temp.cpus[i].processes[total].name,"U");
                ...
                for(q=0;q<numberOfProcesses;q++){
                    ...
                    if(arrive_times[q]<= total && q != i && burst_times[q] !=0 && first_times[q]<0){
                        strcpy(temp.cpus[q].processes[total].name,"W");
                        if(q != previous &&searchQueue(temp.queues[total],processes[q],numberOfProcesses) == 0 && total !=0){
                            strcpy(temp.queues[total].processes_waiting[temp.queues[total].lastIndex].name,processes[q].name);
                            //Increment lastIndex
                            temp.queues[total].lastIndex++;
                        }
                    }
                    //If a process has also arrived earlier but was used before is put as sleeping and entered also in queue
                    else if(arrive_times[q] <= total && q!=i && burst_times[q] !=0 && first_times[q] >= 0){
                        strcpy(temp.cpus[q].processes[total].name,"S");
                        if(q != previous &&searchQueue(temp.queues[total],processes[q],numberOfProcesses) == 0 && total !=0){
                            strcpy(temp.queues[total].processes_waiting[temp.queues[total].lastIndex].name,processes[q].name);
                            //Increment lastIndex
                            temp.queues[total].lastIndex++;
                    ...
                    if(previous >= 0 && previous != i && searchQueue(temp.queues[total],processes[previous],numberOfProcesses) == 0 && burst_times[previous] != 0){
                        strcpy(temp.queues[total].processes_waiting[temp.queues[total].lastIndex].name,processes[previous].name);
                        //Increment lastIndex
                        temp.queues[total].lastIndex++;
                    }
        ...
        //Case 2: process has remaining burst_time > time Quantum and also zero(isn't finished)
            ...
            for(u=0;u<timeQuantum;u++){
                ...
                for(w=0;w<numberOfProcesses;w++){
                    ...
                    if(arrive_times[w]<= total && w!=i && burst_times[w] !=0 && first_times[w]<0){
                        //Update value of virtualCPU to W for waiting
                        strcpy(temp.cpus[w].processes[total].name,"W");
                        if(w != previous &&searchQueue(temp.queues[total],processes[w],numberOfProcesses) == 0 && total !=0){
                            //Add process to queue
                            strcpy(temp.queues[total].processes_waiting[temp.queues[total].lastIndex].name,processes[w].name);
                            //Increment lastIndex of queue
                            temp.queues[total].lastIndex++;
                        }
                    }
                    //If a process has arrived, isnt't finished and has entered the CPU previously
                    else if(arrive_times[w] <= total && w!=i && burst_times[w] !=0 &&first_times[w] >= 0){
                        //Update value of virtualCPU to W for waiting
                        strcpy(temp.cpus[w].processes[total].name,"S");
                        if(w != previous && searchQueue(temp.queues[total],processes[w],numberOfProcesses) == 0 && total !=0) {
                            //Add process to queue
                            strcpy(temp.queues[total].processes_waiting[temp.queues[total].lastIndex].name,processes[w].name);
                            //Increment lastIndex of queue
        ...
        if(burst_times[i] == 0 && counter == 1){
            ...
            temp.waiting_times[i] = total - arrive_times[i] - copyBurst[i];
            temp.ta_times[i] = total - arrive_times[i];
            counter = 0;
        }
        //Find index of next process to be used
        int r;
        for(r=0;r<numberOfProcesses;r++){
            if(strcmp(processes[r].name,temp.queues[total-1].processes_waiting[0].name) == 0){
                next = r;
            }
        }
